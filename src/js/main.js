require([
    ...
    portalSelf.util.fixUrl(portalInfo.portalUrl).then(function(portalUrl) {
      ...
      esriId
        ...
        .then(function(data) {
          ...
          if (
            !app.portals.destinationPortal ||
            app.portals.destinationPortal.portalUrl !== portalInfo.portalUrl
          ) {
            app.portals.destinationPortal = new portalSelf.Portal({
              portalUrl: portalInfo.portalUrl,
  ...
  var inspectContent = function() {
      ...
      if (codeBlock.attr("contentEditable") !== "true") {
        ...
        codeBlock.bind(eventType, function() {
          ...
          if (jsonValid === true) {
            ...
            saveButton.attr("data-toggle", "tooltip");
            saveButton.attr("data-placement", "bottom");
            saveButton.attr("title", jsonValid);
          }
    ...
    jquery(".content").click(function() {
      ...
      portal.itemDescription(id).then(function(description) {
        checkData(id, description.name).then(function(data) {
          if (data) {
            itemData = data;
          }
          var templateData = {
                ...
                jquery.each(layers, function(i, v) {
                  ...
                  portal
                    ...
                    .then(function(records) {
                      ...
                      var count =
                        definition.layers[layerId].maxRecordCount < 1
                          ? 1000
  ...
  var highlightCopyableContent = function() {
    var setMaxWidth = function(el) {
      ...
      var maxWidth = jquery("#itemsArea .in").width()
        ...
        : jquery("#itemsArea").width() ? $("#itemsArea").width() - 49 : 400;
  ...
  var listSearchItems = function(portalUrl, results) {
    ...
    jquery.each(results.results, function() {
      var templateData = {
        ...
        agoType:
          this.type +
          (this.typeKeywords.indexOf("Hosted Service") >= 0 ? " (Hosted)" : "")
      };
      var html = mustache.to_html(
        jquery("#contentTemplate").html(),
        templateData
      );
      jquery("#collapse_search")
        .append(html)
        .addClass("in");
    });
  ...
  var listUserItems = function() {
    ...
    var refreshBtn = jquery(
      '<a class="pull-right" href="#">Refresh <span class="glyphicon glyphicon-refresh" aria-hidden="true"></span>'
    );
    ...
    portal.userContent(portal.username, "/").then(function(content) {
      ...
      jquery.each(content.items, function(item) {
        var templateData = {
          ...
          agoType:
            this.type +
            (this.typeKeywords.indexOf("Hosted Service") >= 0
              ? " (Hosted)"
              : "")
        };
        var html = mustache.to_html(
  ...
  jquery(document).ready(function() {
    ...
    jquery("[data-action='logindestination']").click(function() {
      ...
      esriId
        ...
        .then(function(user) {
          ...
          if (
            !app.portals.destinationPortal ||
            app.portals.destinationPortal.portalUrl !== appInfo.portalUrl
          ) {
            app.portals.destinationPortal = new portalSelf.Portal({
              portalUrl: user.server + "/",
              username: user.userId,
    ...
    jquery(document).on("click", "#searchMenu li", function(e) {
      var selectedAction = jquery(e.target)
        .parent()
        .attr("data-action");
      if (
        selectedAction !== "viewMyContent" &&
        selectedAction !== "viewMyGroups"
      ) {
    ...
    jquery("#currentUrl").text(
      window.location.origin + window.location.pathname
    );
    jquery("#currentUrl2").text(
      window.location.origin + window.location.pathname
    );
  });
});
